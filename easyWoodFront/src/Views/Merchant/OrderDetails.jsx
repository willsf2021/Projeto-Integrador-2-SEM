...
const OrderDetail = ({ order, onBack }) => {
  ...
  useEffect(() => {
    const fetchAttachments = async () => {
      try {
        setLoading(true);
        const data = await getOrderAttachments(currentOrder.id);
        setAttachments(data);
      } catch (err) {
  ...
  const handleUpdateOrder = async (e) => {
    ...
    try {
      setLoading(true);
      const updatedOrder = await updateMerchantOrder(currentOrder.id, {
  ...
  const handleDeleteOrder = async () => {
    if (window.confirm('Tem certeza que deseja excluir este pedido?')) {
      try {
        setLoading(true);
        await deleteMerchantOrder(currentOrder.id);
        onBack();
      } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    try {
      setLoading(true);
      const newAttachment = await uploadAttachment(currentOrder.id, file);
  ...
  const handleDownload = async (attachmentId, filename) => {
    try {
      const response = await downloadAttachment(currentOrder.id, attachmentId);
  ...
  const handleDeleteAttach = async (attachmentId) => {
    if (window.confirm('Tem certeza que deseja excluir este anexo?')) {
      try {
        await deleteAttachment(currentOrder.id, attachmentId);
